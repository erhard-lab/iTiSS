...
public class TsrFile {
    ...
    public TsrFile merge(TsrFile other, int extension) {
        ...
        for (TsrFileEntry entry : other.loop()) {
            ...
            List<TsrFileEntry> overlaps = EI.wrap(tmpmem.get(entry.getReference()).iterateIntervalsIntersecting(entry.getOriginalStart()-extension, entry.getOriginalEnd()+extension, i->true)).map(Map.Entry::getValue).list();
